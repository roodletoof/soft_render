...
void sr_pal_reset();
unsigned char sr_pal_transform(unsigned char from);
unsigned char sr_get_pixel( sr_surface *self, int x, int y);
void sr_pixel( sr_surface *self, unsigned char color, int x, int y);
void sr_line( sr_surface *self, unsigned char color, int x0, int y0, int x1, int y1);
void sr_rect_fill( sr_surface *self, unsigned char color, int x, int y, int width, int height);
void sr_rect_outline( sr_surface *self, unsigned char color, int x, int y, int width, int height);
void sr_circle_fill( sr_surface *self, unsigned char color, int x, int y, int radius);
void sr_circle_outline( sr_surface *self, unsigned char color, int x, int y, int radius);
void sr_blit( sr_surface *destination, int dest_x, int dest_y, sr_surface *source);
...
void sr_rect_outline_masked( sr_surface *self, unsigned char color, int x, int y, int width, int height);
void sr_circle_fill_masked( sr_surface *self, unsigned char color, int x, int y, int radius);
void sr_circle_outline_masked( sr_surface *self, unsigned char color, int x, int y, int radius);
void sr_blit_masked( sr_surface *destination, int dest_x, int dest_y, sr_surface *source);
...
static void circle_pixel_masked(sr_surface *self, unsigned char color, int center_x, int center_y, int x, int y) {
    ...
    sr_pixel_masked(self, color, center_x+x, center_y-y);
    sr_pixel_masked(self, color, center_x-x, center_y-y);
    sr_pixel_masked(self, color, center_x+y, center_y+x);
    sr_pixel_masked(self, color, center_x-y, center_y+x);
    sr_pixel_masked(self, color, center_x+y, center_y-x);
    sr_pixel_masked(self, color, center_x-y, center_y-x);
}
...
void sr_pixel_masked(
    ...
    if (
        ...
        if (sr_get_pixel(self, x, y) == self->invisible) { return; }
        self->pixels[y * self->width + x] = color_map[color];
    }
}
...
void sr_line_masked(
    ...
    int error = dx + dy;
    while (1) {
        sr_pixel_masked( self, color, x0, y0 );
        if ( x0 == x1 && y0 == y1 ) {
            break;
        }
        int e2 = 2* error;
...
void sr_rect_outline_masked(
    ...
    for ( int x2 = x; x2 < x + width; x2++ ) {
        sr_pixel_masked( self, color, x2, y );
        sr_pixel_masked( self, color, x2, y + height - 1 );
    }
    for ( int y2 = y+1; y2 < y + height - 1; y2++ ) {
        sr_pixel_masked( self, color, x, y2 );
        sr_pixel_masked( self, color, x + width - 1, y2 );
    }
}

void sr_circle_fill_masked(
    sr_surface *self,
    unsigned char color,
    int x,
    int y,
...
void sr_blit_masked(
    ...
    for ( int y = start_y; y < stop_y; y++ ) {
        for ( int x = start_x; x < stop_x; x++ ) {
            unsigned char color = sr_get_pixel(source, x-start_x, y-start_y);
            if (color == source->invisible) { continue; }
            sr_pixel_masked(destination, color, x, y);
        }
    }
}

void sr_blit_pro_masked(
    sr_surface *destination,
    int dest_x,
    int dest_y,
    int dest_width,
...
void sr_fill_masked(
    ...
    int buffer_size = self->width * self->height;
    for (int i = 0; i < buffer_size; i++) {
        if (self->pixels[i] == self->invisible) { continue; }
        self->pixels[i] = color_map[color];
    }
}
